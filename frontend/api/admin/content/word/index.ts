...
export async function listWords(params: { page?: number; limit?: number; q?: string; jlpt?: string; sort?: 'updatedAt'|'createdAt'|'termJP'; }) {
  const url = new URL(`${BASE}/admin/words`);
...
export async function getWord(id: string) {
  const res = await fetch(`${BASE}/admin/words/${id}`, { headers: authHeaders() });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function createWord(payload: Word) {
  const res = await fetch(`${BASE}/admin/words`, { method: 'POST', headers: authHeaders(), body: JSON.stringify(payload) });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function updateWord(id: string, payload: Partial<Word>) {
  const res = await fetch(`${BASE}/admin/words/${id}`, { method: 'PUT', headers: authHeaders(), body: JSON.stringify(payload) });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function deleteWord(id: string) {
  const res = await fetch(`${BASE}/admin/words/${id}`, { method: 'DELETE', headers: authHeaders() });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
