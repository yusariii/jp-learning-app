...
const LIMIT = 20;
const DIFFS: Array<Reading['difficulty'] | ''> = ['', 'easy', 'medium', 'hard'];
...
export default function ListReadingScreen() {
  ...
  const fetchPage = useCallback(async (p: number, append = false) => {
    setLoading(true);
    try {
      const res: ApiList = await listReadings({
        page: p,
        limit: LIMIT,
        q: q.trim() || undefined,
    ...
    <LayoutDefault title="Bài đọc">
      <View style={styles.header}>
        ...
        <View style={{ flexDirection: 'row', gap: theme.tokens.space.xs, flexWrap: 'wrap' }}>
          {DIFFS.map(d => (
            <Chip key={d || 'all'} label={d || 'All'} active={(difficulty || '') === d} onPress={() => { setDifficulty(d as any); fetchPage(1); }} />
          ))}
        </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ContentCard>
            ...
            <Text style={[theme.text.secondary, { marginTop: theme.tokens.space.xs }]} numberOfLines={3}>
              {item.textJP}
            </Text>
